...
import GameCharacter.Bomb.Bomb;
import GameCharacter.Bomb.BombAnimation;
...
public class BomberCharacter extends Character {
    ...
    private GameSound soundMoving = new GameSound(GameSound.MOVING_SOUND);
     ...
     */
    public BomberCharacter(int x, int y, KeyboardInput keyboardInput) {
        super(x, y, GameSprite.playerDown.getFxImage());        // mặc định là image playerDown
        direction = 1;
        velocity = 2;
        input = keyboardInput;
    }

    public void restoreBomber(BomberCharacter newBomber) {
        reset();
    ...
    public void update() {
        ...
        if (isStartDie()) {
            ...
            if (input.space && bombList.size() < maxBom) {
                GameEntity e = BomberManGame.getCanvasGame().getEntityInCoodinate(getXUnit(), getYUnit());
                if (e == null) {
            ...
            if (isMoving()) {
                ...
                if (!soundMoving.isRunning()) {
                    soundMoving.play_sound();
                }
                calculateMove();
            } else {
                soundMoving.stop_sound();
            }
        }
    }
    ...
    public void calculateMove() {
        ...
        if (input.up) {
            ...
            setDirection(3);
            this.setImg(GameSprite.movingSprite(GameSprite.grass1.playerRight, GameSprite.playerRight1, GameSprite.grass1.playerRight2,
                    animation, timeTransfer).getFxImage());
        }
    }
...
//            GameEntity e = BomberManGame.getCanvasGame().getEntityInCoodinate(newX, newY);
//
...
//            int newX = (getX() + addToXToCheckCollision[i]) / GameSprite.SCALED_SIZE;
//            int newY = (getY() + addToYToCheckCollision[i]) / GameSprite.SCALED_SIZE;
//            GameEntity collision = BomberManGame.getCanvasGame().getEntityInCoodinate(newX, newY);
//            int preX = (getX() + addToXToSetPrecision[i]) / GameSprite.SCALED_SIZE;
//            int preY = (getY() + addToYToCSetPrecision[i]) / GameSprite.SCALED_SIZE;
//            GameEntity check = BomberManGame.getCanvasGame().getGrass(preX, preY);
            ...
            for (BombAnimation f : fl) {
                if (f.getXUnit() == x && f.getYUnit() == y) {
                    if (!canPassFlame && !startDie) {
                        setStartDie(true);
                        new GameSound(GameSound.DEAD_SOUND).play_sound();
